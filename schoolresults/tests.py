...
class CompilationTests(TestCase):
    def student(self, reg_no, class_level):
        return Student.objects.create(first_name="Test",last_name=reg_no,reg_no=reg_no,class_level=class_level,term="Third",session="2026/2027",term_ending=date(2027,7,20),next_term_begins=date(2027,9,10))
    def result(self, student, subject, first_ca, second_ca, exam):
        return Result.objects.create(student=student,subject=subject,first_ca=first_ca,second_ca=second_ca,exam=exam,first_term=60,second_term=70)
    def test_grade_duplicate_and_tie_ranking(self):
        subject=Subject.objects.get(name="MATHEMATICS",section="primary")
        ...
        with tempfile.TemporaryDirectory() as media:
            with override_settings(MEDIA_ROOT=media):
                compilation=compile_class("Basic 3")
    ...
    def test_nursery_template_mapping(self):
        subject=Subject.objects.get(name="MATHEMATICS",section="nursery")
        student=self.student("N1","Nursery 1"); self.result(student,subject,10,10,30)
        with tempfile.TemporaryDirectory() as media:
            with override_settings(MEDIA_ROOT=media):
                compile_class("Nursery 1"); student.refresh_from_db()
